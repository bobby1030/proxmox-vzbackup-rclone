...
destpath="${REMOTE_NAME}:${SUB_PATH}${timepath}"
...
if [[ ${COMMAND} == 'rehydrate' ]]; then
    ...
    --drive-chunk-size=32M copy ${destpath}/$rehydrate$CMDARCHIVE $dumpdir \
...
if [[ ${COMMAND} == 'backup-end' ]]; then
    ...
    #ls $rclonedir
    rclone --config ${CONFIG_PATH} \
    --drive-chunk-size=32M copy $tarfile ${destpath} \
    -v --stats=60s --transfers=16 --checkers=16
fi
...
if [[ ${COMMAND} == 'job-end' ||  ${COMMAND} == 'job-abort' ]]; then
    ...
    #ls $rclonedir
    rclone --config ${CONFIG_PATH} \
    --drive-chunk-size=32M move $_filename4 ${destpath} \
    -v --stats=60s --transfers=16 --checkers=16
